// RUN: clang-tidy -load=%tidy_plugin -checks='-*,pg-list-free' %s -- | FileCheck %s --check-prefix=TIDY
...
void my_func(List *list) {
    pfree(list); 
    // TIDY: :[[@LINE-1]]:5: warning: Applying pfree() on a list is not allowed. Use list_free() instead. [pg-list-free]
